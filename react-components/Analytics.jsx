...
const Analytics = () => {
  const [data, setData] = useState({ automated_corrections: 0, estimated_savings_kwh: "0.0000" });
  ...
  return (
    <div style={{ display: 'flex', gap: '2rem', marginTop: '1rem' }}>
      ...
      <div style={{ flex: 1, backgroundColor: '#fff', borderRadius: '1rem', padding: '1.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', textAlign: 'center' }}>
        <h4 style={{ color: '#6b7280', margin: 0, fontSize: '0.9rem', textTransform: 'uppercase' }}>Estimated Savings</h4>
        <div style={{ fontSize: '3rem', fontWeight: 'bold', color: '#111827', marginTop: '0.5rem' }}>
          {data.estimated_savings_kwh} <span style={{ fontSize: '1rem', color: '#9ca3af' }}>kWh</span>
        </div>
        <p style={{ margin: 0, color: '#3b82f6', fontSize: '0.85rem', fontWeight: 600 }}>Total Energy Saved</p>
      </div>
    </div>
  );
};
